    ...
    def explain_global(self, model_id: str, training_data_file: Optional[str] = None,
        ...
        try:
            ...
            if training_data_file:
                ...
                feature_file = metadata.get('feature_file')

                logger.info(f"模型元数据: {metadata}")
                logger.info(f"从元数据中获取的feature_file: {feature_file}")

                if feature_file:
                    # 规范化文件路径
                    if not os.path.isabs(feature_file):
                        feature_file = os.path.join(settings.BASE_DIR, feature_file)

                    logger.info(f"规范化后的feature_file路径: {feature_file}")
                    logger.info(f"文件是否存在: {os.path.exists(feature_file)}")

                    if os.path.exists(feature_file):
                        logger.info(f"从模型元数据中加载训练数据: {feature_file}")
                        df = pd.read_csv(feature_file)
                        training_data = df.values.astype(np.float64)
                    else:
                        logger.warning(f"模型元数据中的特征文件不存在: {feature_file}")
                        training_data = None
                else:
                    logger.warning(f"未提供训练数据文件，且模型元数据中也没有特征文件路径")
                    training_data = None
            ...
            if training_data is not None:
                ...
                logger.warning(f"未提供训练数据，无法计算准确的全局SHAP值")
                raise ValueError(
                    "无法计算全局特征重要性：缺少训练数据。"
                    "请提供 training_data_file 参数，或确保模型元数据中包含 feature_file 路径。"
                )
